...
class TextDataset(Dataset):
    ...
    def to_one_hot(self, user, input):
    ...
    def from_one_hot(self, input):
        return torch.argmax(input).data
...
class RNN(nn.Module):
    def __init__(self):
        ...
        self.RNN = nn.RNN(input_size=17, hidden_size=64, num_layers=3, bidirectional=False, batch_first=True, dropout=0.15)
...
class CustomLossFunction():
    ...
    def calculate(self, output, expected):
        ...
        difference = torch.multiply(difference, self.loss_weights)
...
def train(model, epochs, learning_rate, training_data):
    ...
    for i in range(0, epochs):
        ...
        for j in range(0, current_batch.shape[0]):
            if random() > 0.5:
                for k in range(0, current_batch[j].shape[0]):
                    if current_batch[j][k][1] == 1:
                        current_batch[j][k][1] = 0
                        current_batch[j][k][2] = 1
                    else:
                        if current_batch[j][k][2] == 1:
                            current_batch[j][k][1] = 1
                            current_batch[j][k][2] = 0
